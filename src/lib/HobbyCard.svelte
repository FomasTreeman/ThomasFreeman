...
>
	<div class="hobby-content" bind:this={content}>
		<div class="hobby-icon-wrapper">
			...
			<div class="icon-ring" style="--hobby-color: {color}" />
		...
		<h4>{title}</h4>
		<div class="hobby-glow" style="--hobby-color: {color}" />
		<div class="bubbles">
			{#each Array(6) as _, i}
				<div class="bubble" style="--delay: {i * 1.5}s; --size: {Math.random() * 2 + 1}rem" />
			{/each}
		</div>
	</div>
</li>
...
<style>
	...
	.bubble {
		position: absolute;
		background: radial-gradient(circle at center, var(--hobby-color) 0%, transparent 70%);
		border-radius: 50%;
		width: var(--size);
		height: var(--size);
		opacity: 0;
